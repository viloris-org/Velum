...
The Velum release installer installs the companion automatically. For a source
build, install it separately, then set the provider-specific environment
variables documented by Lego in a mode-`0600` environment file. Do not put
those values in `config.toml` or command arguments.
...
the generated certificate and key, and stages both replacements before
activation. If the admin socket exists, activation retains the previous pair
until a confirmed local reload and restores it if reload fails. On first
issuance, no service is running yet, so activation commits the pair for the
next start. A process crash between the two file replacements still requires
operator recovery; this is not a multi-file atomic filesystem update.
...
The guided `velum setup` flow collects the ACME account, relay DNS name, and
DNS provider, or lets the operator choose an existing or self-signed pair.
...
the active PEM files untouched. A successful renewal activation requires the
running service's local admin socket; start or restart the relay before running
the first renewal.
