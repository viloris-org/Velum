...
| S2-03 | Minimal server and selected local adapter | `PARTIAL` | [`velum-adapter-connect`](../crates/velum-adapter-connect) supplies the selected IP-only CONNECT parser; [`velum-node`](../apps/velum-node) composes exact-target admission with a QUIC stream relay and provides a live Quinn listener accepting independent bidirectional flows. Its experimental CLI loads PEM TLS material, exact targets, quotas, and credentials, then handles bounded shutdown; local-adapter wiring remains |
