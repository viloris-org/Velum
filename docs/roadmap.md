...
| S2-03 | Minimal server and selected local adapter | `PARTIAL` | [`velum-adapter-connect`](../crates/velum-adapter-connect) supplies the selected IP-only CONNECT parser; [`velum-node`](../apps/velum-node) composes exact-target admission with a QUIC stream relay and provides a live Quinn listener accepting independent bidirectional flows. Deployment-owned certificate provisioning and local-adapter wiring remain |
