...
pub fn random_listener_bind() -> Result<String, String> {
    for _ in 0..128 {
        let mut bytes = [0_u8; 2];
        getrandom::fill(&mut bytes)
            .map_err(|error| format!("cannot choose listener port: {error}"))?;
        let port = 49_152 + (u16::from_le_bytes(bytes) % 16_384);
        if listener_port_is_available(port) {
            return Ok(format!("0.0.0.0:{port}"));
        }
    }
    Err("cannot find an available TCP and UDP listener port after 128 attempts".into())
}

fn listener_port_is_available(port: u16) -> bool {
    let Ok(_udp) = std::net::UdpSocket::bind(("0.0.0.0", port)) else {
        return false;
    };
...
pub fn write(path: &Path, config: &Config) -> Result<(), String> {
    ...
    let cover_service_template =
        COVER_SERVICE_TEMPLATE.replace("0.0.0.0:4433", &config.listener.bind);
...
pub(crate) fn read_secret(path: &Path) -> Result<Vec<u8>, String> {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mode = fs::metadata(path)
...
mod tests {
    ...
    #[test]
    fn generated_listener_bind_uses_the_dynamic_port_range() {
        for _ in 0..32 {
            let bind = random_listener_bind().expect("listener bind");
            let port = bind
                .rsplit_once(':')
    ...
    #[test]
    fn listener_port_probe_rejects_an_existing_udp_listener() {
        let listener = std::net::UdpSocket::bind(("0.0.0.0", 0)).expect("UDP listener");
        let port = listener.local_addr().expect("UDP address").port();

        assert!(!listener_port_is_available(port));
    }

    #[test]
    fn listener_port_probe_rejects_an_existing_tcp_listener() {
        let listener = std::net::TcpListener::bind(("0.0.0.0", 0)).expect("TCP listener");
